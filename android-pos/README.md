...
## Tablet app trong kiến trúc 3 app

`android-pos` chính là **tablet app shell**. Nó không tạo database riêng và không lưu dữ liệu bán hàng riêng trong Android.

- Desktop app: mở web shell với `?app=desktop`.
- Tablet app: Android WebView này mở `?app=tablet`.
- Phone app: mở web shell với `?app=phone` hoặc tự nhận theo màn hình nhỏ.

Ba app khác giao diện và quyền hiển thị tính năng, nhưng cùng truy cập một POS Engine:

```text
Desktop / Tablet Android / Phone
        -> http://<IP-may-POS>:3000
        -> auth session + branch/store context
        -> SQLite/local engine + Socket.IO realtime
```

Nếu cùng đăng nhập vào cùng cửa hàng/chi nhánh, dữ liệu sẽ đồng bộ qua cùng API và realtime channel. Ví dụ: tablet tạo order, KDS và desktop POS thấy ngay; desktop bán retail thì phone/tablet cùng chi nhánh thấy tồn kho cập nhật.

Luật UI hiện tại:

- `tablet`: có iPad Self-Order và có nút camera scan.
- `desktop`: không hiện iPad Self-Order, không hiện nút camera scan; vẫn dùng máy quét USB/nhập barcode.
- `phone`: không hiện iPad Self-Order, vẫn có camera scan.
