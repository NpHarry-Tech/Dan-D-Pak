...
# BACKEND-SCOPED deploy gate. Decoupled from the client/hardware release gate
# (verify-production-evidence.ps1) per owner direction: a server image cutover +
# DB migration with rollback does not depend on Flutter/APK/Authenticode signing,
# physical hardware canaries, manual acceptance or store-edge WAN canaries.
#
# NOTHING backend-relevant is relaxed. This gate still enforces, at full strength:
#   clean commit + host pin, verified encrypted backup + decryptability,
#   restore rehearsal (schema 7->8, quick_check ok, ZERO money-path orphans,
#   no table/row loss, outbox preserved), server image provenance, the full
#   canonical server suite, and pinned rollback image readiness.
# The excluded checks (flutter/windows/phone/tablet signing, hardwareCanary,
# manualAcceptance, storeEdgeWan, business reconciliation) remain MANDATORY for
# the separate client/device rollout — they are release-readiness, not the
# mechanical safety of swapping the backend container.
$ErrorActionPreference = 'Stop'
$root = Split-Path -Parent $PSScriptRoot
if (-not (Test-Path -LiteralPath $Evidence -PathType Leaf)) { throw "NO_GO: evidence file does not exist: $Evidence" }
...
if (-not [DateTimeOffset]::TryParse([string]$gate.createdAtUtc, [ref]$created)) { throw 'NO_GO: createdAtUtc is missing or invalid.' }
$age = [DateTimeOffset]::UtcNow - $created.ToUniversalTime()
if ($age.TotalHours -lt -0.25 -or $age.TotalHours -gt $MaxAgeHours) { throw "NO_GO: evidence is outside the allowed $MaxAgeHours-hour window." }

function Require-True([string]$Name, $Value) { if ($Value -ne $true) { throw "NO_GO: $Name is not proven true." } }
function Require-Sha([string]$Name, $Value) { if ([string]$Value -notmatch '^[0-9a-fA-F]{64}$') { throw "NO_GO: $Name is not a SHA-256." } }
function Require-Minimum([string]$Name, $Value, [int]$Minimum) {
  $number = 0
  if (-not [int]::TryParse([string]$Value, [ref]$number) -or $number -lt $Minimum) { throw "NO_GO: $Name must be at least $Minimum." }
}

if ([int]$gate.formatVersion -ne 1) { throw 'NO_GO: unsupported evidence format.' }
if ([string]$gate.gateScope -ne 'backend') { throw 'NO_GO: this evidence is not scoped backend.' }
...
if ([string]$gate.restoredBackupSha256 -ne [string]$gate.rehearsalSourceBackupSha256) { throw 'NO_GO: rehearsed DB bytes do not match the decrypted production backup.' }
...
if ([string]$gate.databaseQuickCheckResult -ne 'ok') { throw 'NO_GO: restored production copy PRAGMA quick_check did not return exactly ok.' }
...
if (-not [int]::TryParse([string]$gate.pendingOutboxBefore, [ref]$pendingBefore) -or $pendingBefore -lt 0 -or
    ...
    $pendingBefore -ne $pendingAfter) { throw 'NO_GO: pending outbox counts before/after restore do not match.' }
...
if ([string]::IsNullOrWhiteSpace([string]$gate.rollbackImage)) { throw 'NO_GO: rollbackImage is missing.' }
if ([string]$gate.rollbackImageId -notmatch '^sha256:[0-9a-f]{64}$') { throw 'NO_GO: rollbackImageId must pin the rehearsed immutable Docker image ID.' }
if ([string]$gate.pinnedHostKeyFingerprint -ne 'SHA256:fmDCv6ehU4KpbB+pV7uVvFbC+M0SM6OF8YINwuAkRZM') { throw 'NO_GO: pinned SSH host-key fingerprint mismatch.' }
