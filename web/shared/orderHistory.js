...
import { danBillVars, danPromoText } from './danBill.js';
...
const METHOD_VN = { cash: 'Tiền mặt', bank: 'Chuyển khoản', card: 'Thẻ', bank_transfer: 'Chuyển khoản', qrcode: 'QR', qr: 'QR', momo: 'MoMo', zalopay: 'ZaloPay', visa: 'Visa', voucher: 'Voucher' };
...
function isLegacyBillTemplate(tpl) {
  ...
  return !!tpl && (tpl.standard !== 'dan_payment_receipt' || Number(tpl.version || 0) < 6);
}
...
function formatReceiptItemRow(idx, i, rate, priceIncludesVat) {
  ...
  const row = `${String(idx).padStart(2, '0')}   ${name} ${qty} ${vndPlain(unit).padStart(9, ' ')} ${vndPlain(amount).padStart(12, ' ')}`;
...
function receiptVarsBcm(r) {
  ...
  const res = {
    ...
    invoiceSeries: ein.series || '',
    taxInvoiceNo: r.tax_invoice_no || r.invoice?.invoice_no || '',
    billNo,
    orderNo: billNo,
    ...
    customerName: r.customer?.name || 'Bán cho người tiêu dùng',
    ...
    taxAuthorityCode: r.tax_authority_code || r.invoice?.lookup_code || '',
...
function renderBasicReceipt(r) {
  ...
  return `<div class="receipt">
      ...
      ${(r.items || []).map(i => {
        const promo = danPromoText(i.promo);
        return `<div class="li"><span>${i.qty}× ${esc(i.name)}${promo ? `<small>${esc(promo)}</small>` : ''}</span><span>${money((i.unit_price || i.price || 0) * (i.qty || 1))}</span></div>`;
      }).join('')}
      <hr>
...
function receiptHtml(r) {
  ...
  const rows = itemRows(r, (it, stt, groupLabel) => {
    ...
    const promo = danPromoText(it.promo);
    return `<tr><td class="oh-stt">${String(stt).padStart(2, '0')}</td><td>${esc(it.name)}${mods ? `<small class="oh-mods">+ ${esc(mods)}</small>` : ''}${promo ? `<small class="oh-mods"><i>KM: ${esc(promo)}</i></small>` : ''}<small class="oh-mods">${it.qty} × ${money(it.unit_price)}</small></td><td class="r">${money(it.line_total)}</td></tr>`;
  });
  const inv = r.invoice;
...
function printReceipt(r) {
  ...
  if (tpl?.elements?.length) {
    ...
    const rows = itemRows(r, (it, stt, groupLabel) => {
      ...
      const mods = (it.mods || []).map(m => m.label || m.name).filter(Boolean).join(', ');
      const promo = danPromoText(it.promo);
      return `<tr><td class="stt">${String(stt).padStart(2, '0')}</td><td>${esc(it.name)}${mods ? `<small>+ ${esc(mods)}</small>` : ''}${promo ? `<small><i>KM: ${esc(promo)}</i></small>` : ''}<small>${it.qty} × ${money(it.unit_price)}</small></td><td class="r">${money(it.line_total)}</td></tr>`;
    });
    w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${esc(r.bill_no || r.number)}</title>
      <style>
        *{margin:0;padding:0;box-sizing:border-box}
...
function injectCss() {
  ...
  s.textContent = `
  .oh-overlay{position:fixed;inset:0;z-index:950;background:rgba(26,34,48,.52);backdrop-filter:blur(6px);display:none;align-items:center;justify-content:center;padding:20px}
  .oh-overlay.show{display:flex}
  .oh-modal{background:var(--surface);border:1px solid var(--border);border-radius:var(--r-lg);width:100%;max-width:880px;height:min(82vh,720px);display:flex;flex-direction:column;box-shadow:0 18px 54px rgba(20,33,47,.16);overflow:hidden}
