...
const APP_MODE_KEY = 'dan_app_mode';
const APP_MODES = new Set(['desktop', 'tablet', 'phone']);
const BRANCH_ESTABLISHED_KEY = 'branch_established';   // thiết bị đã được gán/chốt chi nhánh chưa

function readRequestedAppMode() {
  const params = new URLSearchParams(location.search);
  const requested = String(params.get('app') || params.get('mode') || params.get('device') || '').toLowerCase();
  if (!APP_MODES.has(requested)) return '';
  localStorage.setItem(APP_MODE_KEY, requested);
  return requested;
}
...
export function getAppMode() {
  const requested = readRequestedAppMode();
  if (requested) return requested;
  const stored = String(localStorage.getItem(APP_MODE_KEY) || '').toLowerCase();
  if (APP_MODES.has(stored)) return stored;
  return inferAppMode();
}

export const isDesktopApp = () => getAppMode() === 'desktop';
export const isTabletApp = () => getAppMode() === 'tablet';
export const isPhoneApp = () => getAppMode() === 'phone';
export const canUseCameraScan = () => !isDesktopApp();
export const canOpenIpadSelfOrder = () => true;
...
window.addEventListener('resize', () => {
  if (!APP_MODES.has(String(localStorage.getItem(APP_MODE_KEY) || '').toLowerCase())) applyAppMode();
});
...
export const canOpenModule = (keyOrModule) => {
  ...
  if (m.key === 'ipad' && !canOpenIpadSelfOrder()) return false;
...
export function requestPinCode(opts = {}) {
  ...
  return new Promise(resolve => {
    ...
    const fail = (message) => {
      ...
      try { navigator.vibrate?.([55, 25, 55]); } catch {}
      pin = '';
      draw();
...
function injectPinCodeCss() {
  ...
  .pin-cancel{position:absolute;top:14px;left:15px;background:transparent;border:0;color:var(--brand);font-size:13px;font-weight:750;padding:7px 8px;border-radius:10px}
  .pin-cancel:hover{background:var(--brand-dim)}
  ...
  .pin-dots i.on{background:var(--brand);border-color:var(--brand);box-shadow:0 0 0 5px var(--brand-dim)}
  ...
  .pin-key:hover,.pin-key:focus-visible{border-color:var(--brand);background:#eefbff;outline:none}
...
function injectLoginCss() {
  ...
  .lg-card{background:var(--surface);border:1px solid var(--border);border-radius:var(--r-lg);padding:26px;width:100%;max-width:420px;box-shadow:0 18px 54px rgba(20,33,47,.14)}
...
export function connect(device, handlers = {}) {
  const wrapped = Object.fromEntries(Object.entries(handlers || {}).map(([event, fn]) => [event, (...args) => {
    markNotifyHot();
    return typeof fn === 'function' ? fn(...args) : undefined;
  }]));
  const s = connectRealtime(device, getBranchId(), wrapped, setOnline, getToken() || '');
  startPingMonitor();
  return s;
}
...
function markNotifyHot() {
  const bell = document.getElementById('notifyBell');
  if (!bell) return;
  bell.classList.add('hot');
  clearTimeout(_notifyTimer);
  _notifyTimer = setTimeout(() => bell.classList.remove('hot'), 4200);
}
...
export function topbar(active, opts = {}) {
  ...
  Promise.resolve().then(() => startPingMonitor());
  return `<header class="topbar">
    ...
    <div class="tb-right">
      ...
      <button type="button" class="tb-iconbtn notif-bell" id="notifyBell" title="${getLang() === 'en' ? 'Realtime notifications' : 'Thông báo realtime'}" aria-label="${getLang() === 'en' ? 'Realtime notifications' : 'Thông báo realtime'}">🔔</button>
...
export function renderUserChip() {
  ...
  el.innerHTML = `<span class="tb-userchip">
    👤 ${esc(u.name || u.username || '')} <small>· ${esc(ROLE_LABEL2[u.role] || u.role || '')}</small>
    <button id="logoutBtn" type="button" title="${getLang() === 'en' ? 'Logout' : 'Đăng xuất'}">⏻</button></span>`;
  document.getElementById('logoutBtn').onclick = () => logout();
  const bell = document.getElementById('notifyBell');
  if (bell) bell.onclick = () => toast(getLang() === 'en' ? 'Realtime notifications are on.' : 'Thông báo realtime đang bật.');
}
...
export async function openCameraScanner(callback) {
  if (!canUseCameraScan()) {
    toast('Desktop app dùng máy quét USB hoặc nhập mã vạch bằng bàn phím.', true);
    return;
  }
  injectScannerCss();
