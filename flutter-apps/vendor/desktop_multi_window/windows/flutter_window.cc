...
FlutterWindow::FlutterWindow(
  ...
  // The POS customer display is a kiosk surface. Creating it as WS_POPUP is
  // materially safer than changing WS_OVERLAPPEDWINDOW after Flutter's child
  // view is attached (which dispatches non-client messages during startup).
  persistent_window_ =
      args.find("\"route\":\"customer_display\"") != std::string::npos;
  ...
  HWND window_handle = CreateWindow(
      ...
      persistent_window_ ? WS_POPUP : WS_OVERLAPPEDWINDOW,
...
LRESULT FlutterWindow::MessageHandler(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) {
  ...
  switch (message) {
    ...
    case WM_DESTROY: {
      Destroy();
      destroyed_ = true;
      // Do not erase the owning unique_ptr from inside this object's WndProc.
      // desktop_multi_window 0.2.1 did that synchronously via OnWindowDestroy,
      // deleting `this` while MessageHandler was still on the stack.
      return 0;
    }
    case WM_CLOSE: {
      // The customer display engine is intentionally reused. A native close
      // button must never destroy it; hiding also makes Alt+F4 harmless.
      if (persistent_window_) {
