...
class DashboardTab extends StatefulWidget {
  ...
  const DashboardTab({super.key, required this.api, required this.refresh});
...
class _DashboardTabState extends State<DashboardTab> {
  static const _trendKeys = ['byDay', 'byWeek', 'byMonth', 'byQuarter', 'byYear'];
  static const _trendLabels = ['Ngày', 'Tuần', 'Tháng', 'Quý', 'Năm'];
  static const _trendSub = {
    'byDay': '7 ngày gần nhất',
    'byWeek': '8 tuần gần nhất',
    'byMonth': '12 tháng gần nhất',
    'byQuarter': '8 quý gần nhất',
    'byYear': '5 năm gần nhất',
  };
  ...
  Widget build(BuildContext context) {
    ...
    if (_error != null && _data == null) {
      return Padding(
        ...
        child: InlineMessage('Không tải được số liệu ($_error)',
  ...
  Widget _kpiRow(DashboardData d, double maxWidth) {
    final cards = [
      ...
      KpiCard(label: 'Bill trung bình', value: Fmt.money(d.avg)),
      KpiCard(
          label: 'Đơn đang mở',
  ...
  Widget _windowNote(DashboardWindow w) {
    ...
    final endText = w.closed && w.end != null ? Fmt.dmyHm(w.end!) : 'hiện tại';
    final text = w.isShift
        ? 'Doanh thu được tính từ lúc mở ca đầu ngày $startText đến $endText.'
        : 'Chưa có ca được mở hôm nay, tạm tính theo ngày lịch từ $startText đến $endText.';
    return Text(text,
        style: const TextStyle(
            fontSize: 12.5, color: DanColors.muted, fontWeight: FontWeight.w500));
  }
    ...
    return Panel(
      ...
      child: d.topItems.isEmpty
          ? const Padding(
              ...
              child: Center(
                  child: Text('Chưa bán món nào',
                      style: TextStyle(color: DanColors.faint))),
            )
          : Column(
              children: [
                const _TopItemHeader(),
                const Divider(height: 14, color: DanColors.border),
                for (var i = 0; i < d.topItems.length; i++) ...[
                  _TopItemRow(rank: i + 1, item: d.topItems[i]),
                  if (i < d.topItems.length - 1)
                    const Divider(height: 12, color: DanColors.border),
                ],
              ],
            ),
    );
  ...
  Widget _hourPanel(DashboardData d) {
    ...
    return Panel(
      ...
      child: VerticalBarChart(
          bars: bars, emptyText: 'Chưa có doanh thu hôm nay'),
    );
  }
  ...
  Widget _trendPanel() {
    ...
    final series = _trends?.range(key) ?? const [];
        ...
        children: [
          Row(
            ...
            children: [
              ...
              _trendStat('TB/KỲ', Fmt.moneyShort(avg)),
              const SizedBox(width: 16),
              _trendStat('CAO NHẤT', Fmt.moneyShort(maxV > 1 ? maxV : 0)),
            ],
          ),
          const SizedBox(height: 14),
          VerticalBarChart(
              bars: bars, emptyText: 'Chưa có doanh thu trong kỳ này'),
        ],
      ),
    );
  }
  ...
  Widget _payAndChannelPanel(DashboardData d) {
    final timeRef = d.window.isShift ? '(Ca hôm nay)' : '(Trong ngày)';
    ...
    return Panel(
      title: 'Phương thức thanh toán  $timeRef',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          if (d.methods.isEmpty)
            ...
            for (final m in d.methods)
              StatBarRow(
                ...
                value: m.amount,
                total: methodTotal,
                color: methodColors[m.method] ?? const Color(0xFF34D2EE),
                valueText: Fmt.money(m.amount),
              ),
          const SizedBox(height: 16),
          const Text('Doanh thu theo kênh',
          ...
          if (channels.isEmpty)
            ...
            for (final e in channels)
              StatBarRow(
                ...
                value: e.value,
                total: channelTotal,
                color: channelColors[e.key] ?? const Color(0xFF34D2EE),
  ...
  Widget _stationPanel(DashboardData d) {
    const stations = ['kitchen', 'bar', 'salad', 'beverage'];
    const labels = {
      'kitchen': '🍳 Bếp',
      'bar': '🍸 Bar',
      'salad': '🥗 Salad',
    ...
    int countFor(String s) =>
        d.stations.firstWhere((x) => x.station == s,
            orElse: () => const StationLoad('', 0)).count;

    return Panel(
      title: 'Tải các station',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
  ...
  Widget _lowStockPanel(DashboardData d) {
    return Panel(
      title: '⚠️ Cảnh báo tồn kho (${d.lowStock.length})',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  Text('${Fmt.int0(s.stock)} / ${Fmt.int0(s.minStock)} ${s.unit}',
                      style: TextStyle(
                          fontSize: 12.5,
                          fontWeight: FontWeight.w800,
                          color: s.stock <= 0 ? DanColors.late : DanColors.doing)),
                ],
              ),
            ),
        ],
...
class _TopItemHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return const Row(
      children: [
        Expanded(child: Text('MÓN', style: style)),
        SizedBox(width: 56, child: Text('SL', style: style, textAlign: TextAlign.right)),
        SizedBox(
            width: 120,
      ...
      children: [
        ...
        SizedBox(
          ...
          child: Text(
            ...
            textAlign: TextAlign.right,
            style: const TextStyle(
                fontSize: 13, fontWeight: FontWeight.w800, color: DanColors.brand),
          ),
        ),
      ],
    );
