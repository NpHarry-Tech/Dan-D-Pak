...
class _PrintTemplateDesignerState extends State<PrintTemplateDesigner> {
  final _saveDebounce = _Debouncer(const Duration(milliseconds: 700));
  ...
  Map<String, dynamic> _defaultBill() {
    ...
    return {
      ...
      'rows': [
        ...
        _lineRow(),
        _tRow('HÓA ĐƠN THANH TOÁN', align: 'center', bold: true),
        _tRow('Số bill: {billNo}\n{place}\nThu ngân: {cashier}\nNgày: {time}'),
        _lineRow(),
        _tRow('{items}'),
        _lineRow(),
        _tRow(
  ...
  void _scheduleSave() {
    ...
    setState(() => _saveState = 'Đang chờ lưu');
  }
        ...
        Expanded(
          child: Wrap(
            ...
            children: [
              ...
              _modeButton('label', 'Tem nhãn', Icons.label_outline),
              _paperDropdown(),
              IconButton(
                onPressed: _showTokenList,
                icon: const Icon(Icons.info_outline, color: DanColors.muted),
                tooltip: 'Danh sách dữ liệu (token)',
              ),
            ],
          ),
        ),
    ...
    return Container(
      ...
      child: SingleChildScrollView(
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            _section('KHỔ IN', _paperControls()),
            if (_kind == 'bill') _section('THÔNG TIN CỬA HÀNG', _storeInfo()),
            _section('NỘI DUNG MẪU', _rowsEditor()),
            _section('CHÈN DỮ LIỆU', _tokenPalette()),
          ],
        ),
      ),
    );
  ...
  List<Widget> _paperControls() {
    return [
      Row(children: [
        Expanded(
            child: _numberField('Rộng mm', _d(_template['widthMm']),
      ...
      Row(children: [
        ...
        const SizedBox(width: 8),
        Expanded(
            child: _numberField('Số bản', _d(_media['copies'], 1),
      ...
      if (_kind == 'label') ...[
        ...
        Row(children: [
          Expanded(
              child: _numberField('Lề trái', _d(_media['rollMarginLeftMm']),
                  (v) => _setMediaValue('rollMarginLeftMm', v))),
          const SizedBox(width: 8),
          Expanded(
              child: _numberField('Lề phải', _d(_media['rollMarginRightMm']),
  ...
  List<Widget> _storeInfo() {
    ...
    return [
      field('Tên cửa hàng', 'storeName'),
      field('Dòng mô tả', 'storeSubtitle'),
      field('Địa chỉ', 'address', maxLines: 2),
      Row(children: [
        Expanded(child: field('MST', 'taxCode')),
        const SizedBox(width: 8),
        Expanded(child: field('SĐT', 'phone')),
      ]),
      field('Email', 'email'),
      field('Lời cảm ơn (footer)', 'footer', maxLines: 2),
    ];
  }
    ...
    return [
      if (rows.isEmpty)
        const Padding(
          ...
          child: Text('Chưa có dòng nào — bấm “+ Dòng chữ” bên dưới.',
      ...
      Wrap(
        ...
        runSpacing: 8,
        children: [
          _addBtn(Icons.text_fields, 'Dòng chữ', () => _addRow('text')),
          _addBtn(Icons.horizontal_rule, 'Đường kẻ', () => _addRow('line')),
          _addBtn(
              Icons.table_rows_outlined, 'Bảng món', () => _addRow('items')),
          _addBtn(Icons.image_outlined, 'Logo/Ảnh', () => _addRow('image')),
          _addBtn(Icons.qr_code_2, 'Mã QR', () => _addRow('qr')),
          OutlinedButton.icon(
            onPressed: _restoreDefault,
            icon: const Icon(Icons.restart_alt, size: 18),
            label: const Text('Khôi phục mẫu chuẩn'),
        ...
        child: Column(
          ...
          children: [
            ...
            TextFormField(
              ...
              decoration:
                  const InputDecoration(labelText: 'Dữ liệu', isDense: true),
              onChanged: (v) =>
          ...
          children: [
            ...
            Row(children: [
              ...
              OutlinedButton.icon(
                ...
                label: Text(hasImage ? 'Đổi ảnh' : 'Chọn ảnh logo'),
              ),
              if (hasImage) ...[
                const SizedBox(width: 6),
                TextButton(
            ...
            Text(
              hasImage
                  ? 'Máy in nhiệt in logo dạng chữ [${_s(row['label']).isEmpty ? 'Logo' : _s(row['label'])}]; ảnh dùng để xem/tham chiếu.'
      ...
      children: [
        TextField(
          ...
          decoration: const InputDecoration(
            ...
            hintText: 'Nhập chữ hoặc chèn {dữ liệu}…',
          ),
  ...
  List<Widget> _tokenPalette() {
    final groups = _kind == 'bill'
        ? const {
            ...
            'Món & tiền': [
              ['{items}', 'Bảng món'],
              ['{subtotalLine}', 'Thành tiền'],
              ['{vatLine}', 'VAT'],
              ['{orderPromoLine}', 'KM toàn bill'],
              ['{grandTotalLine}', 'Tổng cộng'],
              ['{totalLine}', 'Tổng'],
              ['{paymentLines}', 'Thanh toán'],
              ['{paidLine}', 'Đã trả'],
              ['{changeLine}', 'Tiền thối'],
            ],
            'Khác': [
              ['{footer}', 'Lời cảm ơn'],
              ['{invoiceLookupUrl}', 'Link tra cứu'],
            ],
          }
        : const {
            'Tem nhãn': [
                  ...
                  for (final tk in entry.value)
                    Tooltip(
                      ...
                      child: OutlinedButton(
                        ...
                        child:
                            Text(tk[1], style: const TextStyle(fontSize: 12.5)),
                      ),
                    ),
                ],
              ),
            ...
            child: SingleChildScrollView(
              ...
              child: Center(
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: Colors.white,
                    border: Border.all(color: const Color(0x33102840)),
                    boxShadow: const [
                      BoxShadow(
  ...
  List<Widget> _previewWidgets() {
    ...
    if (widgets.isEmpty) widgets.add(_pvText('(mẫu trống)', 'center', false));
    return widgets;
  }
  ...
  Widget _pvLogoPlaceholder(String label) {
    return Container(
      ...
      decoration: BoxDecoration(
        border: Border.all(color: const Color(0x55102840)),
        borderRadius: BorderRadius.circular(6),
      ),
      child: Text(
  ...
  Map<String, String> get _billSample => {
        ...
        'items': '2x Trà đào 60.000đ\n1x Bánh cookie 30.000đ',
        'total': '90.000đ',
        'grandTotal': '90.000đ',
        'totalLine': 'TỔNG: 90.000đ',
        'paymentLines': 'Tiền mặt: 100.000đ',
        'paidLine': 'Đã trả: 100.000đ',
        'changeLine': 'Tiền thối: 10.000đ',
        'method': 'Tiền mặt',
  ...
  Map<String, String> get _labelSample => const {
        'orderNo': 'A01-023',
        'billNo': 'A01-023',
        'table': 'A01',
        'channel': 'Mang đi',
  ...
  void _showTokenList() {
    ...
    showDialog<void>(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('Danh sách dữ liệu (token)'),
        content: SizedBox(
          width: 420,
          child: SingleChildScrollView(
            child: Column(
