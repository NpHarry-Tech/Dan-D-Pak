...
const _roles = ['owner', 'manager', 'cashier', 'kitchen', 'warehouse'];
            ...
            return Container(
              ...
              child: Row(
                children: [
                  ...
                  TextButton(
                      onPressed: () => _openForm(u), child: const Text('Sửa')),
                  if (_s(u['role']) != 'owner')
                    TextButton(
                      onPressed: () => _delete(u),
                      style:
  ...
  Future<void> _save() async {
    if (_name.text.trim().isEmpty || _username.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Cần nhập tên và tên đăng nhập'),
    ...
    if (!_isEdit && !RegExp(r'^\d{4}$').hasMatch(pinText)) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Mã PIN phải đúng 4 chữ số'),
          backgroundColor: DanColors.late));
      return;
    }
...
class BranchesPanel extends StatefulWidget {
  final ApiService api;
  const BranchesPanel({super.key, required this.api});
            ...
            return Container(
              ...
              child: Row(
                children: [
                  ...
                  _Pill(_b(b['active']) ? 'Đang mở' : 'Đã đóng',
                      _b(b['active']) ? DanColors.done : DanColors.muted),
                  TextButton(
                      onPressed: () => _openForm(b), child: const Text('Sửa')),
                ],
              ),
            );
          },
...
class _BranchFormDialog extends StatefulWidget {
  ...
  const _BranchFormDialog({required this.api, this.branch});
...
class _BranchFormDialogState extends State<_BranchFormDialog> {
  ...
  Future<void> _save() async {
    if (_name.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Cần nhập tên chi nhánh'),
          backgroundColor: DanColors.late));
      return;
    }
    final body = {
  ...
  Widget build(BuildContext context) {
    return SideSheetScaffold(
      title: _isEdit ? 'Sửa chi nhánh' : 'Thêm chi nhánh',
      actions: [
        TextButton(
            onPressed: () => Navigator.of(context).pop(),
            child: const Text('Hủy')),
      ...
      child: Column(
        ...
        children: [
          ...
          TextField(
              controller: _code,
              decoration: const InputDecoration(labelText: 'Mã chi nhánh')),
          const SizedBox(height: 12),
          AddressFields(
            address: _address,
...
class TablesPanel extends StatefulWidget {
  final ApiService api;
  const TablesPanel({super.key, required this.api});
...
class _TablesPanelState extends State<TablesPanel> {
  ...
  Widget build(BuildContext context) {
    ...
    for (final t in _tables) {
      zones
          .putIfAbsent(_s(t['zone']).isEmpty ? 'Khác' : _s(t['zone']), () => [])
          .add(t);
    }

    return SettingsPanelScaffold(
      title: 'Cấu hình bàn & Sơ đồ',
      addLabel: 'Thêm bàn',
      onAdd: () => _openForm(),
      onRefresh: _load,
      child: settingsState(
...
class _TableFormDialog extends StatefulWidget {
  final ApiService api;
  final Map<String, dynamic>? table;
  const _TableFormDialog({required this.api, this.table});
...
class _TableFormDialogState extends State<_TableFormDialog> {
  ...
  Future<void> _save() async {
    if (_zone.text.trim().isEmpty || _code.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Cần nhập khu vực và số bàn'),
          backgroundColor: DanColors.late));
      return;
    }
    final pin = await settingsPin(
        context,
        _isEdit
            ? 'Cập nhật bàn "${_code.text.trim()}".'
            : 'Tạo bàn "${_code.text.trim()}".');
    if (pin == null) return;
  ...
  Widget build(BuildContext context) {
    return SideSheetScaffold(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          const Text('XEM TRƯỚC TRÊN SƠ ĐỒ BÀN',
              style: TextStyle(
                  fontSize: 11,
                  fontWeight: FontWeight.w900,
                  letterSpacing: .4,
          ...
          TextField(
              ...
              decoration: const InputDecoration(
                  labelText: 'Khu vực', hintText: 'VD: Tầng 1, Sân vườn')),
          const SizedBox(height: 12),
          TextField(
              controller: _code,
              decoration: const InputDecoration(labelText: 'Số bàn / Mã bàn')),
          const SizedBox(height: 12),
          TextField(
              controller: _seats,
              keyboardType: TextInputType.number,
  ...
  Widget _tablePreview() {
    ...
    return Container(
      ...
      child: Column(
        children: [
          const Icon(Icons.table_restaurant, size: 34, color: DanColors.brand),
