...
class BookMenuPanel extends StatefulWidget {
  ...
  const BookMenuPanel({super.key, required this.api, this.moduleSwitcher});
...
class _BookMenuPanelState extends State<BookMenuPanel> {
  ...
  List<AdminMenuItem> _items = const [];
  bool _loading = true;
  bool _saving = false;
  String? _error;
  String? _bookId;
  ...
  Future<void> _save({bool reload = true}) async {
    ...
    try {
      ...
      if (reload) _toast('Đã lưu menu quyển');
    } catch (e) {
      if (!mounted) return;
  ...
  void _deleteBook() {
    ...
    if (book == null) return;
    if (books.length <= 1) {
      _toast('Cần giữ lại ít nhất một quyển menu', error: true);
      return;
    }
    setState(() {
    ...
    final values = await showDialog<List<String>>(
      ...
      builder: (ctx) {
        ...
        return AlertDialog(
          ...
          title: const Text('Import PubHTML5'),
          content: SizedBox(
            width: 460,
            child: Column(
              ...
              children: [
                TextField(
                    controller: url,
                    decoration:
                        const InputDecoration(labelText: 'Link PubHTML5')),
                const SizedBox(height: 10),
                TextField(
                    controller: title,
                    decoration: const InputDecoration(labelText: 'Tên quyển')),
              ],
            ),
          ),
          actions: [
  ...
  Future<void> _addPage() async {
    final src = await _askText('Thêm trang', 'URL ảnh trang menu',
        initial:
  ...
  Widget build(BuildContext context) {
    ...
    final book = _book();
    if (book == null) {
      return const Center(child: Text('Chưa có cấu hình menu quyển'));
    }
            ...
            children: [
              SizedBox(
                ...
                child: DropdownButtonFormField<String>(
                  ...
                  decoration: const InputDecoration(
                      labelText: 'Quyển đang chỉnh', isDense: true),
                  items: [
              ...
              SizedBox(
                ...
                child: TextFormField(
                  ...
                  decoration: const InputDecoration(
                      labelText: 'Tên quyển', isDense: true),
              ...
              FilledButton.icon(
                ...
                label: const Text('Lưu menu quyển'),
              ),
              OutlinedButton.icon(
                  onPressed: _newBook,
                  icon: const Icon(Icons.add, size: 16),
                  label: const Text('Tạo quyển')),
              OutlinedButton.icon(
                  onPressed: _importPubhtml5,
                  icon: const Icon(Icons.cloud_download_outlined, size: 16),
                  label: const Text('Import PubHTML5')),
              OutlinedButton.icon(
                onPressed: _deleteBook,
                icon: const Icon(Icons.delete_outline, size: 16),
                label: const Text('Xóa quyển'),
                style:
                              ...
                              : ClipRRect(
                                  ...
                                  child: Image.network(
                                    ...
                                    errorBuilder: (_, __, ___) => const Center(
                                      child: Text('Không tải được ảnh trang',
                                          style:
  ...
  Widget _pageStrip(List<Map<String, dynamic>> pages,
    ...
    if (pages.isEmpty) {
      return const Text('Chưa có trang menu',
          style: TextStyle(color: DanColors.faint));
    }
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  if (hs == null)
                    const Text('Chọn một nút để chỉnh món, vị trí và màu.',
    ...
    return Column(
      ...
      children: [
        DropdownButtonFormField<String>(
          ...
          isExpanded: true,
          decoration: const InputDecoration(
              labelText: 'Món được mở khi bấm', isDense: true),
          items: [
            for (final item in _items)
              DropdownMenuItem(
                  value: item.id,
        ...
        TextFormField(
          ...
          decoration:
              const InputDecoration(labelText: 'Nhãn nội bộ', isDense: true),
        ...
        TextFormField(
          ...
          decoration:
              const InputDecoration(labelText: 'Màu nút', isDense: true),
        ...
        Row(
          children: [
            ...
            const SizedBox(width: 8),
            Expanded(child: _numberField(hs, 'angle', 'Góc')),
          ],
        ),
        SwitchListTile(
          ...
          activeThumbColor: DanColors.brand,
          title: const Text('Hiện nút trên tablet/iPad',
