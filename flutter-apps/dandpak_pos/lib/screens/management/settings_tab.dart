...
class _SettingsSection {
  ...
  final String desc;
  final IconData icon;
  const _SettingsSection(this.key, this.label, this.desc, this.icon);
}
...
class _SettingsTabState extends State<SettingsTab> {
  static const _sections = [
    ...
    _SettingsSection('integrations', 'Liên kết',
        'Hóa đơn điện tử, kế toán và nền tảng bán hàng.', Icons.hub_outlined),
    _SettingsSection('connections', 'Kết nối',
        'Trạng thái thiết bị, máy in và đồng bộ cloud.', Icons.cable_outlined),
    _SettingsSection('warehouse', 'Kho & kênh bán',
        'Quản lý kho hàng và liên kết kênh bán.', Icons.warehouse_outlined),
    _SettingsSection('print', 'Bill & Tem nhãn',
        'Thiết kế mẫu in hóa đơn và tem sản phẩm.', Icons.print_outlined),
    _SettingsSection(
        'devices',
        'Thiết bị khách',
  ...
  Widget _content() {
    switch (_selected) {
      ...
      case 'operations':
        return OperationsPanel(api: widget.api);
      case 'connections':
...
class LoyaltyPromotionsPanel extends StatelessWidget {
  final ApiService api;
  const LoyaltyPromotionsPanel({super.key, required this.api});
...
class _SubInProgress extends StatelessWidget {
  final String label;
  final String desc;
  const _SubInProgress({required this.label, required this.desc});
      ...
      child: ConstrainedBox(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              const Icon(Icons.tune, size: 38, color: DanColors.faint),
              ...
              const SizedBox(height: 12),
              const Text('Đang được port trong module này.',
                  style: TextStyle(
