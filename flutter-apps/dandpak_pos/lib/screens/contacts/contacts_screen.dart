...
class _CustomerColumn {
  ...
  final double width;
  final bool numeric;
  const _CustomerColumn(this.key, this.label, this.width,
      {this.numeric = false});
}
...
const _customerColumns = [
  _CustomerColumn('code', 'Mã khách hàng', 132),
  _CustomerColumn('name', 'Tên khách hàng', 220),
  _CustomerColumn('phone', 'Điện thoại', 130),
  _CustomerColumn('type', 'Loại khách hàng', 130),
  _CustomerColumn('group', 'Nhóm khách hàng', 150),
  _CustomerColumn('email', 'Email', 180),
  _CustomerColumn('company', 'Công ty', 190),
  _CustomerColumn('tax', 'Mã số thuế', 130),
  _CustomerColumn('address', 'Địa chỉ', 240),
  _CustomerColumn('currentDebt', 'Nợ hiện tại', 130, numeric: true),
  _CustomerColumn('debtDays', 'Số ngày nợ', 110, numeric: true),
  _CustomerColumn('totalSpent', 'Tổng bán', 135, numeric: true),
  _CustomerColumn('points', 'Điểm hiện tại', 125, numeric: true),
  _CustomerColumn('tier', 'Hạng', 110),
  _CustomerColumn('afterReturn', 'Tổng bán trừ trả hàng', 170, numeric: true),
  _CustomerColumn('status', 'Trạng thái', 120),
];

const _defaultCustomerColumns = {
  'code',
  'name',
  'phone',
  'currentDebt',
...
class _KiotCustomerScreenState extends State<ContactsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: DanModuleTopBar(
        ...
        actions: [
          if (auth.hasPermission('contacts.create'))
            DanTopBarButton(
              onPressed: () => _openForm(),
  ...
  Widget _body() {
    ...
    if (_error != null && _partners.isEmpty) {
      return Padding(
        ...
        child: InlineMessage('Không tải được danh bạ ($_error)',
  ...
  Widget _sidebar() {
    return Container(
      ...
      child: ListView(
        padding: const EdgeInsets.fromLTRB(18, 16, 16, 24),
        children: [
          _filterTitle('Loại đối tác'),
          Wrap(
            spacing: 7,
            runSpacing: 7,
            children: [
          ...
          _dividerGap(),
          _filterTitle('Ngày tạo'),
          _radioLine('Toàn thời gian', true),
          _radioLine('Tùy chỉnh', false,
              trailing: Icons.calendar_month_outlined),
          _dividerGap(),
          _filterTitle('Sinh nhật'),
          _radioLine('Toàn thời gian', true),
          _radioLine('Tùy chỉnh', false,
              trailing: Icons.calendar_month_outlined),
          _dividerGap(),
          _filterTitle('Ngày giao dịch cuối'),
          _radioLine('Toàn thời gian', true),
          _radioLine('Tùy chỉnh', false,
              trailing: Icons.calendar_month_outlined),
          _dividerGap(),
          _filterTitle('Tổng bán'),
          _rangeFields(_salesFrom, _salesTo, onChanged: (_) => setState(() {})),
          _dividerGap(),
          _filterTitle('Nợ hiện tại'),
          _disabledRangeFields(),
          _dividerGap(),
          _filterTitle('Số ngày nợ'),
          _disabledRangeFields(),
          _dividerGap(),
          _filterTitle('Điểm hiện tại'),
          ...
          Wrap(
            ...
            children: [
              ...
              _statusChip('active', 'Đang hoạt động'),
              _statusChip('inactive', 'Ngưng hoạt động'),
            ],
          ),
        ],
      ),
    ...
    return Column(
      children: [
              ...
              SizedBox(
                ...
                child: TextField(
                  decoration: const InputDecoration(
                    hintText: 'Theo mã, tên, số điện thoại',
        ...
        Expanded(
          child: rows.isEmpty
              ? const Center(
                  child: Text('Chưa có khách hàng phù hợp',
  ...
  Widget _tableLine(List<_CustomerColumn> cols, {bool header = false}) {
    return Container(
      height: header ? 42 : 38,
      color: header ? const Color(0xFFEAF4FF) : DanColors.surface,
      child: Row(
        children: [
  ...
  Widget _summaryLine(
      List<_CustomerColumn> cols, List<Map<String, dynamic>> rows) {
    final total =
        rows.fold<int>(0, (sum, c) => sum + _n(c['total_spent']).round());
    final points =
        rows.fold<int>(0, (sum, c) => sum + _n(c['loyalty_points']).round());
    return Container(
      height: 40,
      color: DanColors.surface,
      child: Row(
          ...
          for (final col in cols)
            _cell(
              switch (col.key) {
                'currentDebt' => '0',
                'debtDays' => '',
                'totalSpent' => Fmt.money(total),
                'afterReturn' => Fmt.money(total),
                'points' => Fmt.int0(points),
                _ => '',
              },
              col.width,
              numeric: col.numeric,
        ...
        InkWell(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: selected ? const Color(0xFFF1F8FF) : DanColors.surface,
              border: const Border(
                  top: BorderSide(color: DanColors.border, width: .6)),
            ),
            child: Row(
              children: [
                for (final col in cols)
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              _detailTabButton('info', 'Thông tin'),
              _detailTabButton('address', 'Địa chỉ nhận hàng'),
              _detailTabButton('orders', 'Lịch sử đặt hàng'),
              _detailTabButton('debt', 'Nợ cần thu từ khách'),
              _detailTabButton('points', 'Lịch sử tích điểm'),
              const Spacer(),
          ...
          if (_detailTab == 'orders')
            _emptyDetail('Chưa có lịch sử đặt hàng chi tiết'),
          if (_detailTab == 'debt') _emptyDetail('Khách hàng không có công nợ'),
          ...
          child: Column(
            ...
            children: [
              ...
              Wrap(
                ...
                runSpacing: 14,
                children: [
                  _infoPair('Điện thoại', _s(c['phone'])),
                  _infoPair('Email', _s(c['email'])),
                  _infoPair('Sinh nhật', _s(c['birthday'])),
                  _infoPair('Công ty', _s(c['company'])),
                  _infoPair('Mã số thuế', _s(c['tax_code'])),
                  _infoPair('Địa chỉ', _s(c['address']), wide: true),
                  _infoPair('Ghi chú', _s(c['note']), wide: true),
                ],
              ),
            ],
          ),
  ...
  Widget _addressDetail(Map<String, dynamic> c) {
    if (_s(c['address']).isEmpty &&
      ...
      return _emptyDetail('Chưa có địa chỉ nhận hàng riêng');
    }
    return Wrap(
      spacing: 32,
      runSpacing: 14,
      children: [
        _infoPair(
            ...
            _s(c['address_detail']).isEmpty
                ...
                : _s(c['address_detail']),
            wide: true),
        _infoPair('Phường/Xã', _s(c['address_ward'])),
        _infoPair('Tỉnh/Thành phố', _s(c['address_province'])),
        _infoPair('Mã phường/xã', _s(c['ward_code'])),
        _infoPair('Mã tỉnh/thành', _s(c['province_code'])),
        _infoPair('Dòng đầy đủ', _s(c['address']), wide: true),
      ],
    );
  }
  ...
  Widget _pointsDetail(Map<String, dynamic> c) {
    return Wrap(
      ...
      runSpacing: 14,
      children: [
        _metric('Điểm hiện tại', Fmt.int0(_n(c['loyalty_points']))),
        _metric('Hạng thành viên',
            _s(c['loyalty_tier']).isEmpty ? 'Chưa có' : _s(c['loyalty_tier'])),
        _metric('Tổng đơn', Fmt.int0(_n(c['total_orders']))),
        _metric('Tổng mua', Fmt.money(_n(c['total_spent']))),
        _infoPair(
          'Sở thích / insight',
  ...
  Widget _emptyDetail(String text) {
    return Padding(
      ...
      child: Text(text, style: const TextStyle(color: DanColors.faint)),
    );
  }
    ...
    return SizedBox(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(value.trim().isEmpty ? 'Chưa có' : value,
              maxLines: wide ? 2 : 1,
              overflow: TextOverflow.ellipsis,
  ...
  String _valueFor(Map<String, dynamic> c, String key) {
    ...
    return switch (key) {
      ...
      'name' => _s(c['name']).isEmpty ? 'Khách chưa đặt tên' : _s(c['name']),
      'phone' => _s(c['phone']),
      'type' => _partnerTypeLabel(c),
      'group' => _s(c['perk_type']) == 'none' ? 'Chưa có' : 'Ưu đãi riêng',
      'email' => _s(c['email']),
      ...
      'status' => _b(c['active']) ? 'Đang hoạt động' : 'Ngưng hoạt động',
      _ => '',
    };
  }

  String _partnerTypeLabel(Map<String, dynamic> c) {
    final t = _s(c['partner_type']);
    return switch (t) {
      'supplier' => 'Nhà cung cấp',
      'both' => 'Khách hàng + NCC',
      'staff' => 'Nhân viên',
  ...
  Widget _dividerGap() => const Padding(
  ...
  Widget _rangeFields(TextEditingController from, TextEditingController to,
    ...
    return Column(
      children: [
        _rangeField('Từ', from, onChanged: onChanged),
        const SizedBox(height: 6),
        _rangeField('Tới', to, onChanged: onChanged),
      ],
    );
  }
  ...
  Widget _disabledRangeFields() {
    return Column(
      children: [
        _disabledRangeField('Từ'),
        const SizedBox(height: 6),
        _disabledRangeField('Tới'),
      ],
    );
  }
...
class _ContactsScreenState extends State<_LegacyContactsScreen> {
  ...
  Widget _body() {
    ...
    if (_partners.isEmpty) {
      return const Center(
          child: Text('Chưa có liên hệ nào',
              style: TextStyle(color: DanColors.faint)));
    }
                  ...
                  Row(
                    children: [
                      ...
                      if (isCustomer) ...[
                        ...
                        const _Tag('Khách', DanColors.brand),
                      ],
                      if (isSupplier) ...[
                        const SizedBox(width: 6),
                        const _Tag('NCC', Color(0xFFB45309)),
                      ],
                    ],
                  ),
...
class _PartnerForm extends StatefulWidget {
  ...
  const _PartnerForm({required this.api, this.partner, this.canDelete = false});
...
class _PartnerFormState extends State<_PartnerForm> {
  ...
  void initState() {
    ...
    _perkType = const {'none', 'pct', 'amount', 'free'}.contains(rawPerk)
        ? rawPerk
        : 'none';
  ...
  Future<void> _save() async {
    if (_name.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Cần nhập tên liên hệ'),
          backgroundColor: DanColors.late));
      return;
    }
  ...
  Future<void> _lookupTax() async {
    ...
    setState(() {
      ...
      if (err != null) {
        ...
        final res = _taxLookup.lastResult ?? const {};
  ...
  Future<void> _pickAvatar() async {
    ...
    if (length > 20 * 1024 * 1024) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Ảnh tối đa 20MB'), backgroundColor: DanColors.late));
      return;
    }
  ...
  Future<void> _delete() async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('Xóa "${_name.text.trim()}"?'),
        actions: [
          TextButton(
  ...
  Widget _typeSelector() {
    const options = [
      ('customer', Icons.shopping_bag_outlined, 'Khách hàng'),
      ('supplier', Icons.storefront_outlined, 'Nhà cung cấp'),
      ('both', Icons.people_outline, 'Cả hai'),
      ('staff', Icons.badge_outlined, 'Nhân viên'),
    ];
    return Row(
      children: [
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(_isEdit ? 'Sửa liên hệ' : 'Thêm liên hệ',
                              style: const TextStyle(
                                  fontSize: 19, fontWeight: FontWeight.w900)),
                          const SizedBox(height: 3),
                          const Text('Khách hàng, nhà cung cấp hoặc nhân viên',
                              style: TextStyle(
              ...
              Flexible(
                child: ListView(
                  ...
                  children: [
                    ...
                    _avatarEditor(),
                    _field('MÃ KH', _code, hint: 'Để trống tự sinh DC000001'),
                    ...
                    _field('CÔNG TY', _company,
                        hint: 'Tên công ty (nếu có)',
                        locked: _taxLookup.companyLocked),
                    Row(
                      children: [
                        Expanded(
                          child: _field('SỐ ĐIỆN THOẠI', _phone,
                              hint: '09xx xxx xxx'),
                        ),
                        const SizedBox(width: 12),
                        Expanded(
                    ...
                    if (_partnerType == 'supplier' || _partnerType == 'both')
                      _field('NGƯỜI LIÊN HỆ', _contact,
                          hint: 'Tên người phụ trách bên NCC'),
                    AddressFields(
                      address: _address,
                    ...
                    if (_partnerType == 'customer' ||
                      ...
                      Row(
                        children: [
                            ...
                            child: Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                  Text(
                                    _partnerType == 'staff'
                                        ? 'ƯU ĐÃI MẶC ĐỊNH CHO NHÂN VIÊN'
                                        : 'ƯU ĐÃI MẶC ĐỊNH CHO KHÁCH',
                                    style: const TextStyle(
                                        fontSize: 12,
                                        fontWeight: FontWeight.w700,
                                        color: DanColors.text),
                          ...
                          const SizedBox(width: 12),
                          Expanded(
                            child: _field('Giá trị', _perkValue,
                                number: true, hint: 'Giá trị'),
                          ),
                        ],
                      ),
                    ],
                    _field('GHI CHÚ', _note, lines: 2, hint: 'Ghi chú nội bộ'),
