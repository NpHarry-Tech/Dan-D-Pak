...
class RetailCheckoutDialog extends StatefulWidget {
  ...
  @override
  State<RetailCheckoutDialog> createState() => _RetailCheckoutDialogState();
}

class _RetailCheckoutDialogState extends State<RetailCheckoutDialog> {
  ...
  // PIN xác nhận thủ công (nếu có dòng bank đối soát tay) — gửi kèm checkout.
  String? _manualPin;
  ...
  List<_PayMethod> get _methods {
    ...
    return const [
      _PayMethod('cash', 'Tiền mặt', 'cash', true),
      _PayMethod('bank', 'Chuyển khoản', 'qr', true),
  ...
  bool get _isQr {
    ...
    return m.kind == 'qr' ||
        m.kind == 'wallet' ||
        const ['bank', 'internet_banking', 'qrcode', 'qr', 'momo', 'zalopay']
            .contains(m.key);
  }
  ...
  bool get _isPos {
    final m = _currentMethod;
    return m.kind == 'pos' ||
        const ['card', 'visa', 'pos_card'].contains(m.key);
  }
  ...
  String _defaultReference(String method) {
    ...
    if (const ['card', 'visa', 'pos_card'].contains(method)) {
      return 'POS-$invoice';
    }
    if (const ['bank', 'internet_banking', 'qrcode', 'qr', 'momo', 'zalopay']
        .contains(method)) {
  ...
  Future<void> _checkPayos() async {
    ...
    try {
      ...
      if (retailB(status['paid'])) {
        ...
        _display?.markPaid();
        _toast('Khách đã thanh toán qua payOS');
      } else {
        _toast(
  ...
  Future<void> _chargePos() async {
    ...
    try {
      ...
      if (retailB(result['approved'])) {
        ...
        _toast('Máy POS đã duyệt giao dịch');
      } else {
        _toast(
  ...
  void _addLine(num amount, {String? ref}) {
    if (amount <= 0) {
      _toast('Nhập số tiền > 0', error: true);
      return;
    }
    setState(() {
  ...
  Future<void> _manualConfirmBank() async {
    final amount = _pendingAmount;
    if (amount <= 0) {
      _toast('Không còn số tiền cần thu', error: true);
      return;
    }
  ...
  Map<String, dynamic>? _invoicePayload() {
    ...
    if (!hasAny) throw Exception('Nhập thông tin xuất hóa đơn');
    if (name.isEmpty && company.isEmpty) {
      throw Exception('Nhập tên khách hoặc tên công ty xuất hóa đơn');
    }
  ...
  Future<void> _confirm() async {
    ...
    if (_paid < _payable) {
      _toast('Chưa đủ tiền thanh toán', error: true);
      return;
    }
        ...
        children: [
          Expanded(
            child: Column(
              ...
              children: [
                Text('Thanh toán retail ${widget.invoiceLabel}',
                    style: const TextStyle(
                        fontSize: 19, fontWeight: FontWeight.w900)),
                const SizedBox(height: 3),
                Text('${widget.cart.length} dòng hàng',
                    style: const TextStyle(
        ...
        children: [
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  // Gạt "Xuất hóa đơn công ty" = khách CÓ yêu cầu hóa đơn —
                  // không được hiện "không yêu cầu" nữa.
                  c != null
                      ? c.title
                      : _invoiceEnabled
          ...
          SwitchListTile(
            ...
            title: const Text('Xuất hóa đơn công ty',
  ...
  Widget _summaryBox() {
    return Container(
      ...
      child: Column(
        children: [
          _totalRow('Tạm tính', Fmt.money(widget.subtotal)),
          if (widget.productDiscount > 0)
            _totalRow(
                'Khuyến mãi sản phẩm', '-${Fmt.money(widget.productDiscount)}',
                accent: DanColors.doing),
          if (widget.orderDiscount > 0)
            _totalRow(widget.voucher?.name ?? 'Voucher toàn bill',
                '-${Fmt.money(widget.orderDiscount)}',
                accent: DanColors.done),
          if (widget.customerDiscount > 0)
            _totalRow(
                'Ưu đãi khách hàng', '-${Fmt.money(widget.customerDiscount)}',
                accent: DanColors.done),
          const SizedBox(height: 8),
          TextField(
            ...
            decoration: const InputDecoration(
              labelText: 'Điều chỉnh hóa đơn (đ)',
              isDense: true,
            ),
          ...
          if (_adjustment > 0)
            _totalRow('Điều chỉnh hóa đơn', '-${Fmt.money(_adjustment)}',
                accent: DanColors.late),
          const Divider(height: 18, color: DanColors.border),
          _totalRow('Cần thanh toán', Fmt.money(_payable), big: true),
        ],
      ),
    );
  }
            ...
            children: [
              Expanded(
                child: TextField(
                  ...
                  keyboardType: TextInputType.number,
                  decoration: const InputDecoration(
                      labelText: 'Số tiền', isDense: true),
                  onChanged: (_) => setState(() {}),
                ),
              ),
              const SizedBox(width: 8),
              Expanded(
                child: TextField(
                  controller: _refCtrl,
                  decoration: const InputDecoration(
                      labelText: 'Mã tham chiếu / approval', isDense: true),
      ...
      child: Row(
        ...
        children: [
          Container(
            ...
            color: Colors.white,
            child: img.isEmpty
                ? const Icon(Icons.qr_code_2, size: 64, color: DanColors.faint)
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                _miniBankRow('Số tiền', Fmt.money(_pendingAmount)),
                _miniBankRow('Nội dung CK', ref),
                if (_qrError != null) ...[
                  ...
                  children: [
                    ...
                    if (retailS(data?['orderCode']).isNotEmpty)
                      OutlinedButton.icon(
                        ...
                        label: const Text('Kiểm tra payOS'),
                      ),
                    // Ca sự cố: khách quét QR CŨ (đã "Tạo lại QR") hoặc
                    // webhook chậm → đối soát tiền-về + PIN + lý do.
                    OutlinedButton.icon(
                      onPressed: _manualConfirmBank,
                      style: OutlinedButton.styleFrom(
                          foregroundColor: DanColors.doing,
                          side: const BorderSide(color: DanColors.doing)),
  ...
  Widget _posHelper() {
    ...
    return Container(
      ...
      child: Row(
        children: [
          const Icon(Icons.credit_card, color: DanColors.brand),
          ...
          OutlinedButton.icon(
            ...
            icon: _posLoading
                ? const SizedBox(
                    ...
                    height: 15,
                    child: CircularProgressIndicator(strokeWidth: 2))
                : const Icon(Icons.tap_and_play_outlined, size: 16),
        ...
        children: [
          ...
          if (_lines.isEmpty)
            const Padding(
              ...
              child: Text('Chưa có dòng thanh toán',
                  style: TextStyle(color: DanColors.faint)),
            )
          else
            for (var i = 0; i < _lines.length; i++) ...[
              if (i > 0) const Divider(height: 10, color: DanColors.border),
              Row(
                children: [
          ...
          const Divider(height: 18, color: DanColors.border),
          _totalRow('Đã nhận', Fmt.money(_paid)),
          _totalRow(_remain > 0 ? 'Còn thiếu' : 'Tiền thối',
      ...
      child: Row(
        children: [
          ...
          FilledButton.icon(
            ...
            label: Text('Xác nhận ${Fmt.money(_payable)}'),
          ),
        ],
      ),
    );
...
class _PayMethod {
  ...
  const _PayMethod(this.key, this.label, this.kind, this.enabled);
