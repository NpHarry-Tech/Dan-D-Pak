...
class BlackBox {
  ...
  static final List<String> _recent = <String>[]; // ring buffer đính kèm báo lỗi
  ...
  static void init({required String role, ApiService? api}) {
    ...
    try {
      ...
      _writeLine('## START role=$role app=$kAppVersionName+$kAppBuildNumber '
          'os=${Platform.operatingSystemVersion}');
      if (crashTail != null && api != null) {
        _reportPreviousCrash(api, crashTail);
      }
    } catch (e) {
  ...
  static void _reportPreviousCrash(ApiService api, String tail) {
    var attempts = 0;
