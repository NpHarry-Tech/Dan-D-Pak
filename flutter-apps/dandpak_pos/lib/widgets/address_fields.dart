...
class VnProvince {
  ...
  const VnProvince(this.code, this.name, this.wards);
}

class VnWard {
  final String code;
  final String name;
  const VnWard(this.code, this.name);
}
...
class VnAddressParts {
  ...
  final VnWard? ward;
  const VnAddressParts({this.detail = '', this.province, this.ward});
}
...
class VnAddressBook {
  ...
  static Future<VnAddressBook> load() async {
    ...
    final provinces = (json['provinces'] as List? ?? const [])
        .whereType<Map>()
        .map((p) => VnProvince(
              '${p['code']}',
              '${p['name']}',
              (p['wards'] as List? ?? const [])
                  .whereType<Map>()
  ...
  VnAddressParts parse(String input, {VnProvince? preferredProvince}) {
    ...
    if (text.isEmpty) return const VnAddressParts();
    final tokens = text
        .split(',')
        .map((e) => e.trim())
    ...
    final provinceNeedles =
        province == null ? const <String>[] : _unitNeedles(province.name);
    final wardNeedles =
        ward == null ? const <String>[] : _unitNeedles(ward.name);
...
class _AddressFieldsState extends State<AddressFields> {
          ...
          children: [
            TextField(
              ...
              decoration: InputDecoration(
                ...
                hintText: 'Số nhà, tên đường, ấp/tổ/khu phố',
                suffixIcon: IconButton(
                  tooltip: 'Tự tách phường/xã, tỉnh/thành',
  ...
  Widget _wardBox(VnAddressBook? book) {
    final province = book?.provinces
        ...
        .firstOrNull;
    final wards = province?.wards ?? const <VnWard>[];
