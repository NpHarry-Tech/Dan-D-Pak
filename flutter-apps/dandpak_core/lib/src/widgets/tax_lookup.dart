...
class TaxLookupController extends ChangeNotifier {
  ...
  Future<String?> lookup() async {
    ...
    if (tc.isEmpty) return t('Nhập mã số thuế trước khi truy xuất');
    loading = true;
    notifyListeners();
    try {
      ...
      notifyListeners();
      return (res['message'] ?? t('Không tra cứu được thông tin theo MST này.'))
          .toString();
    } catch (e) {
      loading = false;
      notifyListeners();
...
/// The MST input with the t("Truy xuất Cục Thuế") button baked in. Drop this in
...
class MstField extends StatelessWidget {
  ...
  Future<void> _run() async {
    ...
    if (err != null) {
      ...
      onMessage(t('Đã điền thông tin công ty từ dữ liệu Cục Thuế'),
          error: false);
    }
  }
      ...
      builder: (context, _) => TextField(
        ...
        decoration: InputDecoration(
          ...
          helperText: lookup.locked
              ? t('Đã xác thực Cục Thuế — xóa MST để nhập/kiểm tra lại')
              : null,
          helperStyle: TextStyle(fontSize: 10.5, color: DanColors.done),
          suffixIcon: lookup.loading
              ...
              : lookup.locked
                  ...
                  : IconButton(
                      tooltip: t('Truy xuất thông tin công ty từ Cục Thuế'),
                      icon: Icon(Icons.travel_explore, size: 19),
