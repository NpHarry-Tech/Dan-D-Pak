...
int monitorCount() {
  ...
  try {
    ...
    final getMetrics =
        user32.lookupFunction<_GetSystemMetricsC, _GetSystemMetricsD>(
            'GetSystemMetrics');
...
Future<void> makeSecondWindowFullscreen({String title = 'Màn hình phụ'}) async {
  ...
  try {
    ...
    final getMetrics =
        user32.lookupFunction<_GetSystemMetricsC, _GetSystemMetricsD>(
            'GetSystemMetrics');
    final setPos =
        user32.lookupFunction<_SetWindowPosC, _SetWindowPosD>('SetWindowPos');
    final monitorFrom =
        user32.lookupFunction<_MonitorFromWindowC, _MonitorFromWindowD>(
            'MonitorFromWindow');
    ...
    if (getMetrics(_smCmonitors) > 1) {
      ...
      final vx = getMetrics(_smXvirtualscreen),
          vy = getMetrics(_smYvirtualscreen);
      final vw = getMetrics(_smCxvirtualscreen),
          vh = getMetrics(_smCyvirtualscreen);
      int? px, py;
      if (vx + vw > pw) {
        px = pw + 60;
        py = 60; // màn phụ bên phải
      } else if (vx < 0) {
        px = vx + 60;
        py = 60; // màn phụ bên trái
      } else if (vy + vh > ph) {
        px = 60;
        py = ph + 60; // màn phụ phía dưới
      } else if (vy < 0) {
        px = 60;
...
void beginSecondWindowDrag({String title = 'Màn hình phụ'}) {
  ...
  try {
    ...
    final getWindowRect = _user32
        .lookupFunction<_GetWindowRectC, _GetWindowRectD>('GetWindowRect');
...
bool isSecondWindowFullscreen({String title = 'Màn hình phụ'}) {
  ...
  try {
    ...
    final getWindowRect = _user32
        .lookupFunction<_GetWindowRectC, _GetWindowRectD>('GetWindowRect');
    final monitorFrom =
        _user32.lookupFunction<_MonitorFromWindowC, _MonitorFromWindowD>(
            'MonitorFromWindow');
