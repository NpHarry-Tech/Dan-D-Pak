...
class SystemLog {
  ...
  static String _newEventId() =>
      'evt_${DateTime.now().microsecondsSinceEpoch.toRadixString(36)}'
      '${_rand.nextInt(0xFFFFF).toRadixString(36)}';
  ...
  static String? currentCorrelationId() => Zone.current[_zoneKey] as String?;
  ...
  static Map<String, dynamic> _entry({
    ...
    return <String, dynamic>{
      // Stable across queue retries/restarts so the server accepts each event
      // exactly once even when the previous HTTP response was lost.
      'eventId': _newEventId(),
      ...
      'appVersion': AppFlavor.current.versionName,
      'buildNumber': '${AppFlavor.current.buildNumber}',
