...
Add-Type -AssemblyName System.Windows.Forms | Out-Null
$f = New-Object System.Windows.Forms.OpenFileDialog
$f.Filter = 'Ảnh/PDF|*.jpg;*.jpeg;*.png;*.webp;*.gif;*.pdf|Tất cả|*.*'
$f.Title = 'Chọn ảnh hóa đơn'
...
$f.Title = 'Chọn hình ảnh hoặc video quảng cáo'
if ($f.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
  $path = $f.FileName
  $ext = [System.IO.Path]::GetExtension($path).ToLowerInvariant()
  switch ($ext) {
...
Future<String?> _pickAdPathUnix() async {
  ...
  final res = await Process.run('zenity',
      ['--file-selection', '--title=Chọn hình ảnh hoặc video quảng cáo']);
  final out = (res.stdout as String?)?.trim() ?? '';
  return out.isEmpty ? null : out;
}
