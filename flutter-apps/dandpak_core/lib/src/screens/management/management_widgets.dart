...
class VerticalBarChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final maxV = bars.map((b) => b.value).fold<num>(1, (a, b) => b > a ? b : a);
...
class _Bar extends StatelessWidget {
  ...
  final num maxValue;
  final bool peak;
  _Bar({required this.datum, required this.maxValue, required this.peak});

  @override
  Widget build(BuildContext context) {
    final hasRev = datum.value > 0;
    final frac =
        hasRev ? (datum.value / maxValue).clamp(0.06, 1.0).toDouble() : 0.03;
    final color = !hasRev
        ? DanColors.surface3
        : peak
            ? DanColors.brand
...
class InlineMessage extends StatelessWidget {
  ...
  final bool error;
  InlineMessage(this.text, {super.key, this.onRetry, this.error = false});
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          if (onRetry != null) ...[
            SizedBox(height: 10),
            OutlinedButton(onPressed: onRetry, child: Text(t('Thử lại'))),
          ],
        ],
      ),
    );
