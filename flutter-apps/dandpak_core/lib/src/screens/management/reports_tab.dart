...
class ReportsTab extends StatefulWidget {
  ...
  ReportsTab({super.key, required this.api});
...
class _ReportsTabState extends State<ReportsTab> {
  ...
  String get _branchLabel {
    final catalog = _catalog;
    if (catalog == null || catalog.branches.length <= 1) return '';
    if (_allBranches) return t('Tất cả chi nhánh');
    final selected = catalog.branches
        .where((b) => _selectedBranchIds.contains(b.id))
        .toList();
    if (selected.length == 1) return selected.first.name;
    if (selected.isEmpty) return t('Chọn chi nhánh');
    return t('${selected.length} chi nhánh');
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_catalogError != null) {
      return Padding(
        ...
        child: InlineMessage(
            t('Không tải được danh mục báo cáo ($_catalogError)'),
            error: true,
    ...
    if (catalog.reports.isEmpty) {
      return Center(
        child: Padding(
          padding: EdgeInsets.all(40),
          child: Text(t('Tài khoản này chưa có quyền xem báo cáo nào.'),
  ...
  Widget _dropdownPicker(ReportCatalog catalog) {
    return DropdownButtonFormField<String>(
      ...
      isExpanded: true,
      decoration: InputDecoration(labelText: t('Báo cáo')),
      items: [
      ...
      child: Wrap(
        ...
        children: [
          ...
          OutlinedButton.icon(
            ...
            label: Text(t('Từ: ${dateText(_from)}')),
            style: OutlinedButton.styleFrom(minimumSize: Size(0, 36)),
          ),
          OutlinedButton.icon(
            onPressed: () => _pickDate(false),
            icon: Icon(Icons.event, size: 14),
            label: Text(t('Đến: ${dateText(_to)}')),
            style: OutlinedButton.styleFrom(minimumSize: Size(0, 36)),
          ),
  ...
  Widget _previewBody() {
    ...
    if (_reportError != null && _report == null) {
      return Padding(
        padding: EdgeInsets.all(40),
        child: InlineMessage(t('Không tải được báo cáo ($_reportError)'),
      ...
      child: s.rows.isEmpty
          ...
          : SingleChildScrollView(
              ...
              child: DataTable(
                ...
                dataTextStyle: TextStyle(fontSize: 12.5, color: DanColors.text),
                columns: [
...
class _ReportTile extends StatelessWidget {
  ...
  _ReportTile({required this.info, required this.active, required this.onTap});
