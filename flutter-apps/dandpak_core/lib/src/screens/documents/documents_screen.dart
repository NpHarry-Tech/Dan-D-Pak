  ...
  Future<void> _delete(Map<String, dynamic> f) async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text(t('Xóa tài liệu')),
        content: Text(
            'Xóa "${_s(f['original_name']).isEmpty ? _s(f['name']) : _s(f['original_name'])}"?'),
        actions: [
          TextButton(
    ...
    final pin = await requestManagerPin(context, t('Xóa tài liệu vĩnh viễn.'));
    if (pin == null || !mounted) return;
    try {
      await context.read<ApiService>().deleteDocument(_s(f['id']), pin);
      _load();
    } catch (e) {
      _toast(e.toString().replaceFirst('Exception: ', ''), error: true);
    }
            ...
            children: [
              Expanded(
                child: OutlinedButton.icon(
                  ...
                  label: Text(t('Mở'), style: TextStyle(fontSize: 12.5)),
                  style: OutlinedButton.styleFrom(minimumSize: Size(0, 34)),
                ),
              ),
              IconButton(
                onPressed: () => _delete(f),
