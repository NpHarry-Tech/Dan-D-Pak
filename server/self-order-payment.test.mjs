...
db.prepare(`INSERT INTO shifts (id,branch_id,user_name,shift_key,shift_label,opening_cash,status,opened_at) VALUES (?,?,?,?,?,?,?,?)`)
  .run('shift_selforder', 'sala', 'Tester', 'test', 'Test', 0, 'open', now());

function selfOrder() {
  return Orders.createOrUpdateOrder({
    branch_id: 'sala', channel: 'dine_in', source: 'customer_ipad', actor: 'Khach',
...
test('QR của khách tự gọi món lấy số tiền từ server, không từ client', async () => {
  ...
  await assert.rejects(() => Payments.generateCustomerPaymentQr(order.id, {}, 'sala'), /cho nhan vien xac nhan/);

  Orders.confirmPendingItems(order.id, [], 'sala', 'Thu ngan');
  const qr = await Payments.generateCustomerPaymentQr(order.id, {}, 'sala');
...
test('khách bấm "đã chuyển khoản" KHÔNG tự đóng bill — chờ đối tác/nhân viên', () => {
  ...
  const res = Payments.customerQrPay(order.id, { method: 'qrcode' }, 'sala');
...
test('bill TRẢ TRƯỚC MỘT PHẦN vẫn gửi món vào bếp được', () => {
  ...
  db.prepare(`INSERT INTO tables (id,branch_id,zone,code,seats,status) VALUES (?,?,?,?,?,?)`)
    .run('t_TEST9', 'sala', 'Tầng trệt', 'T9', 4, 'free');
  const order = Orders.createOrUpdateOrder({
    branch_id: 'sala', channel: 'dine_in', source: 'staff_pos', table_id: 't_TEST9',
  ...
  const more = Orders.createOrUpdateOrder({
    branch_id: 'sala', channel: 'dine_in', source: 'staff_pos', table_id: order.table_id,
    actor: 'Thu ngan', items: [{ menu_item_id: 'mi_tra', qty: 1 }],
  });
  assert.equal(more.id, order.id);

  // Và gửi bếp được — đây là thao tác từng báo lỗi.
  const confirmed = Orders.confirmPendingItems(order.id, [], 'sala', 'Thu ngan');
  assert.ok(confirmed);
  assert.equal(
