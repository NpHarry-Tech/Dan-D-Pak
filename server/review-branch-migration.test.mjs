...
test('admin PERSIST: owner + active + ["*"]', () => {
  const a = db.prepare(`SELECT role,active,branch_access_json FROM users WHERE username='admin'`).get();
...
test('idempotent: chạy lại seed không lỗi, vẫn review, migration no-op', () => {
