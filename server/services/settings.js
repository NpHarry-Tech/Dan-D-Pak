...
const DEFAULT_PRINT_CONFIG = {
  ...
  bill: {
    ...
    copies: '1',
    printScale: 100,
    footer: 'THANK YOU - SEE YOU AGAIN AT BCM!',
    showQr: '1',
    showTax: '1',
    taxIncludedText: 'Prices include VAT as required by law',
    unitPriceMode: 'vat_included',
    autoPrint: '1',
  },
  printers: [
    { id: 'kitchen', name: 'Kitchen printer', type: 'Kitchen slip', active: true, auto: true },
    { id: 'bar', name: 'Bar printer', type: 'Bar slip', active: true, auto: true },
    { id: 'bill', name: 'Bill printer', type: 'Invoice', active: true, auto: true },
    { id: 'label', name: 'Label printer', type: 'Label', active: true, auto: false },
  ],
  templates: {
    label: null,
    bill: null,
...
function defaultBcmBillText() {
  return [
    ...
    '',
    '                SALES RECEIPT',
    '          (Generated from POS terminal)',
    '',
    'Invoice series: {invoiceSeries}      Tax invoice no.: {taxInvoiceNo}',
    'Bill no. (internal): {billNo}',
    'Date: {date}       Time: {timeOnly}',
    'Cashier: {cashier}    Counter/Table: {table}',
    '',
    'Customer: {customerName}',
    'Tax code: {customerTaxCode}',
    '--------------------------------------------------',
    'No.  Item/Service              Qty  Unit price  Amount',
    '--------------------------------------------------',
    '{items}',
    '--------------------------------------------------',
    'Subtotal:                              {taxableAmount} VND',
    'VAT ({vatRate}%):                      {vatAmount} VND',
    '--------------------------------------------------',
    'TOTAL AMOUNT DUE:                      {grandTotal} VND',
    '--------------------------------------------------',
    ...
    '{taxAuthorityCode}',
    '',
    '(You can look up this invoice at the website:',
    'https://gdt.gov.vn using the seller tax code',
    'and the tax authority code above)',
    '',
    '==================================================',
    ' E-INVOICE GENERATED FROM POS TERMINAL',
    '      {footer}',
  ].join('\n');
}
...
function defaultBcmBillTemplate(bill = DEFAULT_PRINT_CONFIG.bill) {
  ...
  return {
    ...
    widthMm,
    heightMm,
    printerName: bill.printerName || 'Bill printer',
...
export function updateSettings(body = {}, branch_id = 'br1') {
  ...
  if (body.ipad_staff_pin !== undefined) {
    ...
    if (!/^\d{4,8}$/.test(pin)) throw new Error('iPad password must be 4 to 8 digits');
