...
export function registerInventoryRoutes(api, { wrap, guard, guardAny, branch, visibleBranch }) {
  ...
  api.post('/warehouse/transfer', guard('inventory.adjust'), wrap((req) => Inv.transferStock({ ...req.body, created_by: req.user?.name || req.user?.username }, branch(req))));
  ...
  api.get('/warehouse/stocktakes', guard('inventory.adjust'), wrap((req) => Inv.listStocktakes(branch(req), req.query)));
  api.get('/warehouse/stocktakes/:id', guard('inventory.adjust'), wrap((req) => Inv.getStocktakeSession(req.params.id, branch(req))));
  api.post('/warehouse/stocktakes', guard('inventory.adjust'), wrap((req) => Inv.saveStocktakeSession(req.body, branch(req), req.user)));
  api.post('/warehouse/stocktakes/:id/update', guard('inventory.adjust'), wrap((req) => Inv.saveStocktakeSession({ ...req.body, id: req.params.id }, branch(req), req.user)));
  api.post('/warehouse/stocktakes/:id/approve', guard('inventory.adjust'), wrap((req) => Inv.approveStocktakeSession(req.params.id, branch(req), req.user)));
  api.post('/warehouse/stocktakes/:id/cancel', guard('inventory.adjust'), wrap((req) => Inv.cancelStocktakeSession(req.params.id, branch(req), req.user)));
  // Xuất dùng nội bộ: một phiếu XDNB nhiều dòng.
  api.post('/warehouse/internal-use', guard('inventory.adjust'), wrap((req) => Inv.issueInternalUse(req.body, branch(req), req.user)));
  ...
  api.post('/warehouse/price-books', guardAny('warehouse.manage', 'settings.warehouse'), wrap((req) => Inv.savePriceBookMeta(req.body, branch(req), req.user)));
  api.post('/warehouse/price-books/:id/delete', guardAny('warehouse.manage', 'settings.warehouse'), wrap((req) => Inv.deletePriceBookMeta(req.params.id, branch(req), req.user)));
  api.post('/warehouse/price-book/entry', guardAny('inventory.adjust', 'warehouse.manage'), wrap((req) => Inv.setPriceBookEntry(req.body, branch(req), req.user)));
