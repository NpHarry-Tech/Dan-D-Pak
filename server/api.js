...
// Cổng PIN Quản lý/Admin để đổi sang cơ sở khác (chỉ xác minh, KHÔNG tạo session).
// Phát từ trạng thái đang đăng nhập nên dùng guard(); verifyManagerOwnerPin yêu cầu
// owner/manager có quyền vào cơ sở đích.
api.post('/auth/verify-branch-switch', guard(), wrap((req) => {
  const target = req.body?.branch_id;
  if (!target) throw new Error('Thiếu cơ sở đích.');
  const approvedBy = Auth.verifyManagerOwnerPin(req.body?.pin, target);
  if (!approvedBy) throw new Error('Cần PIN Quản lý hoặc Admin (có quyền cơ sở đích) để đổi cơ sở.');
