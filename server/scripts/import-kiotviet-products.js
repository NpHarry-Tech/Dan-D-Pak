...
// KiotViet VAT cell: "8%", "10%", "0%", or "KCT"/"" (không chịu thuế). null = KCT/none.
function parseVat(v) {
  const s = clean(v).toLowerCase();
  if (!s || s === 'kct' || s === 'kcct') return null;
...
function normHeader(h) {
  // fixMojibake BEFORE clean: mojibake for "à"/"ê"/… contains byte 0xA0 (NBSP),
  // which clean()'s \s+ collapse would destroy — breaking UTF-8 recovery.
  return clean(fixMojibake(h)).toLowerCase()
...
// fixMojibake BEFORE clean (see normHeader) so NBSP bytes inside mojibake survive recovery.
function cell(row, idx, field) { const i = idx[field]; return i == null ? '' : clean(fixMojibake(row[i])); }
...
async function run() {
  ...
  const upsert = db.prepare(`
    INSERT INTO skus (id,branch_id,barcode,name,emoji,image,price,cost,stock,min_stock,unit,warehouse_id,category,supplier,source_url,track_lot,expiry_required,active,
                      code,price_pre_tax,vat,brand,group_path,weight,sellable,created_at)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
