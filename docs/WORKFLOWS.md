...
| Static route 404 | Static frontend | Static route not mapped by app server/proxy | Open `/admin` | Served by Express or redirected to `/admin.html` | Missing app/proxy route mapping | None | Express/static config | Verify route after deploy; known case recorded; updated 2026-06-27 |
...
| CORS failure | Split frontend/backend origins | CORS origin missing | Browser calls backend | Clear browser failure; configure env | Wildcard in prod risk | None | CORS config; all API/socket | Set `CORS_ORIGIN`; updated 2026-06-27 |
