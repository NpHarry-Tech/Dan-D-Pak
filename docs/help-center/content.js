    ...
    },
    {
      ...
      sections: [
        ...
        sec('invoice', L('3. Xuất hóa đơn điện tử', '3. Issue an e-invoice', '3. 开具电子发票'), [
          steps(
            ['Khi xác nhận thanh toán, chọn **In hóa đơn** để hoàn tất bill và chỉ in/lưu doanh thu; hệ thống không gửi MISA/HĐĐT.', 'Chọn **In VAT** nếu cần phát hành HĐĐT ngay. Kiểm tra tên người mua/công ty, MST, địa chỉ và email trước khi bấm.', 'Nếu lúc thanh toán chưa xuất VAT, mở **Hóa đơn** hoặc **Lịch sử** sau đó chọn **Xuất VAT** để điền/sửa thông tin và phát hành bù.', 'Xem lại mẫu hóa đơn: mã item, tên hàng/món, đơn vị, số lượng, đơn giá, VAT, giảm giá và tổng.', 'Nếu thông tin sai sau phát hành, không xóa hóa đơn. Dùng quy trình điều chỉnh/thay thế/hủy theo nhà cung cấp HĐĐT và quyền kế toán.'],
            ['At payment confirmation, choose **Print receipt** to complete the bill and only print/save revenue; the system does not send it to MISA/e-invoice.', 'Choose **Issue VAT** when an e-invoice is needed immediately. Verify buyer/company name, tax ID, address and email before submitting.', 'If VAT was not issued at payment, open **Invoices** or **History** later and choose **Issue VAT** to fill/edit buyer data and issue it retroactively.', 'Review item code, item names, units, quantities, prices, VAT, discounts and totals.', 'If incorrect after issuance, never delete it. Follow the provider/accounting adjustment, replacement or cancellation flow.'],
            ['付款确认时选择 **打印账单**：完成账单、打印并保存营收，但不发送到 MISA/电子发票。', '需要立即开具电子发票时选择 **开具VAT**，提交前核对购方/公司名称、税号、地址和邮箱。', '付款时未开VAT的订单，可稍后在 **发票** 或 **历史** 中选择 **开具VAT**，补填/修改购方信息后补开。', '复核商品编码、商品/菜品名称、单位、数量、单价、税率、折扣和总额。', '开具后信息错误不得删除；应按服务商和财务权限执行调整、替换或作废流程。']
          ),
