...
`POST /api/skus/:id/update` accepts `units` as an array. Each unit has `name`,
`factor` (number of base units), optional `barcode`, `cost`, `price`,
`price_includes_vat`, and `vat`. Purchase lines send the selected unit in
`unit`; receiving converts quantity and unit cost to the SKU base unit.

`POST /api/skus/image-upload` accepts the shared base64 image payload
(`original_name`, `mime_type`, `data`) and requires `warehouse.item` or
`inventory.adjust`.
...
| `GET /api/print/printers` | List printer routes with real status. `live=1` probes LAN printers; `system` routes always report the owning POS's agent liveness. Adds `owner_device_id`, `owner_device_name`, `attached_to_me`. Callers without printer-management permission (`settings.printers` / `settings.connections` / `settings.print` / `settings.manage` / Admin) receive only routes plugged into their own `x-device-id` plus shared LAN routes |
| `POST /api/print/printers/:id/test` | Create and dispatch a test print for one configured route. `403` when the route is plugged into another POS and the caller does not manage printers |
| `POST /api/print/cash-drawer/open` | Send an ESC/POS cash drawer pulse through the bill printer of the calling device. `403` when an explicitly requested route belongs to another POS |
...
| `POST /api/print/jobs/:id/print` | Force-print a job. With `PRINT_DISPATCH=agent` the job is re-queued (claim cleared, `print:new` emitted) for the store's Hardware Agent instead of being printed from the VPS |
