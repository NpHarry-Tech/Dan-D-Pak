...
|---|---|---|---|
| Server full regression | **VERIFIED (source)** | 143/143 files; 726 pass; 0 fail/timeout/error; 1,265.3 s | `final-server-test.log`; `TEST_TIMEOUT_MS=240000 TEST_CONCURRENCY=1 node scripts/run-backend-tests.mjs` |
...
| Transaction/migration recovery | **VERIFIED (source)** | atomic rollback, WAL recovery, backup and contention suites included in the 726-pass matrix | server matrix and `baseline-and-runtime.md` |
